...
c
      logical ldouble,lshift,ldiscard,luniform,l1uniform,
     &        open_e,open_w,open_s,open_n
...
c
      call ncopen('../grid_out/grid.nc',ncid)
      call ncgetvar(ncid,'pdepth',depths)
      call ncclose(ncid)
c
c --- land mask
c
      ...
      enddo
c
c --- handle all region types (nreg = closed:0; periodic i-index:1;
c --- arctic:2; doubly periodic:3; periodic j-index:4)
c
      open_e=sum(ip(1  ,1:jdm)).gt.0
      open_w=sum(ip(idm,1:jdm)).gt.0
      open_s=sum(ip(1:idm,1  )).gt.0
      open_n=sum(ip(1:idm,jdm)).gt.0
c
      if     (.not.open_e.and..not.open_w.and.
     &        .not.open_s.and..not.open_n) then
c
        write(6,'(/ a /)') 'closed basin detected'
c
        ip(:,    0) = 0
        ip(:,jdm+1) = 0
        ip(    0,:) = 0
        ...
        nreg = 0
c
      elseif (     open_e.and.     open_w.and.
     &        .not.open_s.and..not.open_n) then
c
        write(6,'(/ a /)') 'periodic basin in i-index detected'
c
        ip(:,    0) = 0
        ip(:,jdm+1) = 0
        ip(    0,:) = ip(idm,:)
...
c
      elseif (     open_e.and.     open_w.and.
     &        .not.open_s.and.     open_n) then
c
        write(6,'(/ a /)') 'basin with arctic patch detected'
c
        if     (luniform) then
          if     (mod(npe,2).ne.0) then
            write(6,'(/ a /)')
     &        'error - grid with arctic patch must have npe even'
            call zhflsh(6)
            stop
          endif
        elseif (npe.ne.1.or.mpe.ne.1) then
          write(6,'(/ a /)')
     &      'error - grid with arctic patch must use equal-size tiles'
          call zhflsh(6)
          stop
        endif
c
...
c
      elseif (     open_e.and.     open_w.and.
     &             open_s.and.     open_n) then
c
        write(6,'(/ a /)') 'doubly periodic basin detected'
c
        ip(1:idm,    0) = ip(1:idm,jdm)
        ip(1:idm,jdm+1) = ip(1:idm,  1)
        ip(    0,:) = ip(idm,:)
        ip(idm+1,:) = ip(  1,:)
        nreg = 3
c
      elseif (.not.open_e.and..not.open_w.and.
     &             open_s.and.     open_n) then
c
        write(6,'(/ a /)') 'periodic basin in j-index detected'
c
        ip(    0,:) = 0
        ip(idm+1,:) = 0
        ip(:,    0) = ip(:,jdm)
...
c
        ...
        write(6,'(/ a /)')
     &      'error - combination of open boundaries not supported'
        call zhflsh(6)
        stop
      endif
c
...
c
     ...
     &            (ip(i-1,j).gt.0.and.ip(i  ,j-1).gt.0)    ) then
          ...
          if (ip(i-1,j-1).gt.0.and.ip(i,j-1).gt.0) then
            map(i,j)=1  ! pvtrop and lateral momentum flux for u
          endif
          if (ip(i-1,j-1).gt.0.and.ip(i-1,j).gt.0) then
            map(i,j)=1  ! pvtrop and lateral momentum flux for v
          endif
          if (ip(i-1,j+1).gt.0.and.ip(i,j+1).gt.0) then
            map(i,j)=1  ! lateral momentum flux for u
          endif
          if (ip(i+1,j-1).gt.0.and.ip(i+1,j).gt.0) then
            map(i,j)=1  ! lateral momentum flux for v
          endif
        enddo
      enddo
c
...
c
      ...
      elseif (nreg.eq.3) then
        map(1:idm,1-mbdy:0)        = map(1:idm, jdm-mbdy+1:jdm)
        map(1:idm, jdm+1:jdm+mbdy) = map(1:idm,          1:mbdy)
        map(1-mbdy:0,       :) = map( idm-mbdy+1:idm, :)
        map( idm+1:idm+mbdy,:) = map(          1:mbdy,:)
      else
        map(1:idm,1-mbdy:0)        = map(1:idm, jdm-mbdy+1:jdm)
...
*           jjpe(mpe  -j/2) = jdm/mpe + 1  ! edge   tiles larger
        ...
        enddo
c
c ---   discard all-land tiles if sfudge greater than 1.0.
c
        nmpe = 0
        do j= 1,mpe
          do i= 1,npe
...
c ---       assume N-S exchange is before E-W exchange.
     ...
     &             sum(map(i_e-mbdy+1:i_e,j_n+1:j_n+mbdy))
            if(nsea.gt.0.or..not.ldiscard) then
              nmpe = nmpe + 1
              ispx(i,j) = ispt(i,j)
              iipx(i,j) = iipe(i,j)
...
c
        if (lshift) then
        do j= 1,mpe
          if     (nreg.eq.2 .and. j.eq.mpe) then
...
c
      call zhopen(21, 'formatted', 'unknown', 0)
